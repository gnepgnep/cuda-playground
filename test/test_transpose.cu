...
int TestTranspose(const int M, const int N, const int method_id = 0, const int run_times = 100) {
    ...
    if (method_id == 0) {
        ...
        TransposeFun1(x.get_data<float>("cuda"), y.get_data<float>("cuda"), M, N, nullptr);
        cudaDeviceSynchronize();
        std::cout << "y: " << y;
        compare_GPUTensor(expect_y, y);        
    }
    ...
    std::vector<GPUTensor *> io_list{&x, &y};
    auto call_fun0 = [&](const std::vector<GPUTensor *> &io_list){
        TransposeFun0(io_list[0]->get_data<float>("cuda"), io_list[1]->get_data<float>("cuda"),
                     M, N, stream);
    };
